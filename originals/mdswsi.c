/* 
   ...
   Written for the course CHET 580 Modern Molecular Simulation
   Spring 2021
...
void xyz_out (FILE * fp, 
  ...
  for (i=0;i<N;i++) {
    fprintf(fp,"%i %.8lf %.8lf %.8lf ",z,
	    rx[i]+(unfold?(ix[i]*Lx):0.0),
	    ry[i]+(unfold?(iy[i]*Ly):0.0),
	    rz[i]+(unfold?(iz[i]*Lz):0.0));
    if (put_vel)
...
double total_e ( double * rx, double * ry, double * rz, 
    ...
    int N, double Lx, double Ly, double Lz, double * E2, double * E3,
    int do_three_body ) {
    ...
    for (j=i+1;j<N;j++) {
      ...
      if (do_three_body==1) {
        ...
        for (k=j+1;k<N;k++) { 
          ...
          printf("eiNjik_y %.8f\n",eiNjik_y);
          printf("eiNjik_z %.8f\n",eiNjik_z);
          printf("ei %.8f dxij %.8f rij %.8e rik %.8e cosjik %.8f dxik %.8f\n",
                  ei,dxij,rij,rik,cosjik,dxik);
          printf("eiNkij_x %.8f\n",eiNkij_x);
          printf("eiNkij_y %.8f\n",eiNkij_y);
          printf("eiNkij_z %.8f\n",eiNkij_z);
          printf("ejNijk_x %.8f\n",ejNijk_x);
          ...
          printf("ekNikj_y %.8f\n",ekNikj_y);
          printf("ekNikj_z %.8f\n",ekNikj_z);
          printf("hij %.8f hik %.8f hjk %.8f\n",hij,hik,hjk);
          printf("hRij_x %.8f dxij %.8f\n",hRij_x,dxij);
          printf("hRij_y %.8f dyij %.8f\n",hRij_y,dyij);
          printf("hRij_z %.8f dzij %.8f\n",hRij_z,dzij);
          printf("hRik_x %.8f dxik %.8f\n",hRik_x,dxik);
          printf("hRik_y %.8f dyik %.8f\n",hRik_y,dyik);
          printf("hRik_z %.8f dzik %.8f\n",hRik_z,dzik);
          printf("hRjk_x %.8f dxjk %.8f\n",hRjk_x,dxjk);
          printf("hRjk_y %.8f dyjk %.8f\n",hRjk_y,dyjk);
          printf("hRjk_z %.8f dzjk %.8f\n",hRjk_z,dzjk);
...
int main ( int argc, char * argv[] ) {
  ...
  double rho=0.46, T0=0.12, vir, vir_old, vir_sum, pcor, V;
  double PE, KE, TE, E2, E3, ecut, TE0, sigma;
  ...
  for (i=1;i<argc;i++) {
    ...
    else if (!strcmp(argv[i],"-Tb")) and_sigma=sqrt(atof(argv[++i]));
    else if (!strcmp(argv[i],"-nu")) and_nu=atof(argv[++i]);
    ...
    else if (!strcmp(argv[i],"-do-three-body")) do_three_body=atoi(argv[++i]);
  ...
  fprintf(stdout,"# nSteps %i, seed %d, dt %.5lf\n",
	  nSteps,Seed,dt);
  if (and_nu>0.0) fprintf(stdout,"# Andersen thermostat, nu %.5lf\n",and_nu); 

  PE = total_e(rx,ry,rz,fx,fy,fz,N,Lx,Ly,Lz,&E2,&E3,do_three_body);
  TE0=PE+KE;
  if (traj_fn) {
    out=fopen(traj_fn,"w");
    xyz_out(out,rx,ry,rz,vx,vy,vz,ix,iy,iz,Lx,Ly,Lz,N,Z_Si,1,unfold);
    fclose(out);
  }

  fprintf(stdout,"#LABELS step time PE KE TE E2 E3 drift T\n");
  ...
  for (s=0;s<nSteps;s++) {
    ...
    TE=PE+KE;
    if (s%prog==0) {
      fprintf(stdout,"%i %.5lf %.5lf %.5lf %.5lf % .5le % .5le % .5le %.5lf\n",
	      s,s*dt,PE/N,KE/N,TE/N,E2/N,E3/N,(TE-TE0)/TE0,KE*2/3./N);
      fflush(stdout);
    }
    if (!(s%fSamp)&&traj_fn) {
      out=fopen(traj_fn,"a");
