...
void write_xyz(FILE * fp, double * rx, double * ry, int n) {
  int i;
  fprintf(fp,"%i\n",n);
  fprintf(fp,"Generated by hdisk.c; all z-components are zero; all elements are H\n");
  for (i=0;i<n;i++) {
    fprintf(fp,"H %.5lf %.5lf 0.00000\n",rx[i],ry[i]);
  }
}
...
int main ( int argc, char * argv[] ) {
  ...
  FILE * tmp_fp;
  int fmt=CUSTOM;
  char * traj_fn_fmt = "traj%05i.dat";
  char * traj_fn = "traj00000.dat";
  ...
  if (!short_out) 
    fprintf(stdout,"# R = %.5lf; rho = %.5lf; N = %i; seed = %lu\n",
	                  sqrt(R2),rho,N,Seed);
  ...
  for (c=0;c<nCycles;c++) {
    ...
    if (traj_samp&&!(c%traj_samp)) {
      if (fmt==CUSTOM) {
        sprintf(traj_fn,traj_fn_fmt,c);
        tmp_fp = fopen(traj_fn,"w");
        out(tmp_fp,rx,ry,N);
        fclose(tmp_fp);
      } else if (fmt==XYZ) {
