...
int main ( int argc, char * argv[] ) {
  ...
  double ei_old, ei_new, delta_e;
  double ivir_old, ivir_new, delta_vir;
  ...
  fprintf(stdout,"# T = %.2lf; rho = % .3lf; N = %i; L = %.5lf; rc = %.5lf\n",
	  T,L,rho,N,sqrt(rc2));
  ...
  for (c=0;c<nCycles;c++) {
    ...
    ei_old=e_i(i,rx,ry,rz,N,L,rc2,tailcorr,ecor,
               shift,ecut,&ivir_old,0);
    ...
    if (c>nEq) {
      ...
      if (prog>0&&!(c%prog)) {
        printf("% 10i % .5f % .5f\n",c,esum/nSamp/N,vir_sum/3.0/V/nSamp+rho/beta);
        fflush(stdout);
      }
  ...
  if (short_out)
    fprintf(stdout,"%.6lf %.5lf %.5lf %.5lf\n",
	    ...
	    esum/nSamp/N,vir_sum/3.0/nSamp/V+rho/beta+pcor);
  else
    fprintf(stdout,"# NVT Metropolis Monte Carlo Simulation"
	    ...
	    ((double)nAcc)/nCycles,
	    esum/nSamp/N,
	    rho/beta,vir_sum/3.0/nSamp/V,
	    vir_sum/3.0/nSamp/V+rho/beta+(tailcorr?pcor:0.0));
  if (traj_fn) {
