...
for f in args.i:
    ...
    ax[0].plot(t,msd,label=f)
    ax[1].plot(np.reciprocal(t[1:]),msd[1:]/6*np.reciprocal(t[1:]),label=f)
    if args.lowt>0.0:
        ...
        print(f,par[0])
        ax[0].plot(t,msd_D(t,*par),'k--')
        ax[1].plot(np.reciprocal(t[1:]),[par[0]]*len(t[1:]),'k--')
ax[0].legend()
ax[1].legend()
