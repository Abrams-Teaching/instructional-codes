...
int ber ( double * vx, double * vy, double * vz, int N, double T, double tau, double dt ) {
  double Tcalc;
  double ksum=0.0;
  double lambda;
  int i;
  ...
  Tcalc=ksum/(3*N);
  lambda=sqrt(1+dt/tau*((T/Tcalc)-1.0));
  for (i=0;i<N;i++) {
    vx[i]*=lambda;
    vy[i]*=lambda;
    vz[i]*=lambda;
...
int main ( int argc, char * argv[] ) {
  ...
  for (i=1;i<argc;i++) {
    ...
    else if (!strcmp(argv[i],"-berT")) berT=atof(argv[++i]);
    else if (!strcmp(argv[i],"-ber_tau")) ber_tau=atoi(argv[++i]);
  ...
  for (s=0;s<nSteps;s++) {
    ...
    KE*=0.5;
    if (berT>0.0) {
      ber(vx,vy,vz,N,berT,ber_tau,dt);
      KE = 0.0;
      for (i=0;i<N;i++) {
        KE+=vx[i]*vx[i]+vy[i]*vy[i]+vz[i]*vz[i];
      }
