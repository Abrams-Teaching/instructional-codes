...
void samp ( double * rx, double * ry, int N, double * theta, int * Rcounts, double R, int nRbins ) {
  int i,j;
  double mx,my,mr,dx,dy,l,cosT,T;
  int rbin;
  for (i=0;i<N;i+=2) {
    mx=(rx[i]+rx[i+1])/2;
    my=(ry[i]+ry[i+1])/2;
    mr=sqrt(mx*mx+my*my);
    rbin=(int)(mr/R*nRbins);
    Rcounts[rbin]++;
    dx=rx[i]-rx[i+1];
    dy=ry[i]+ry[i+1];
    l=sqrt(dx*dx+dy*dy); // should be 1
    cosT=(dx*mx+dy*my)/(mr*l);
    T=acos(cosT);
    theta[rbin]+=cosT*cosT;
  }
}
...
int main ( int argc, char * argv[] ) {
  ...
  double dr=0.1,dx,dy,sx,sy,r2, da=M_PI/2, nx,ny;
  int i,j,k;
  int nCycles = 10;
  int reject;
  ...
  /* allocate order parameter */
  Rcount=(int*)malloc(sizeof(int)*nRbins);
  theta=(double*)malloc(sizeof(double)*nRbins);
  for (i=0;i<nRbins;i++) {
    Rcount[i]=0;
    theta[i]=0.0;
  }
  ...
  for (c=0;c<nCycles;c++) {
    ...
    if (!(c%fSamp)) {
      samp(rx,ry,N,theta,Rcount,R,nRbins);
      if (traj_out==FIG) {
  ...
  tmp_fp=fopen(opf,"w");
  fprintf(tmp_fp,"# Order parameter:\n");
  deltar=R/nRbins;
  for (i=0;i<nRbins;i++) {
    thisr=(i+0.5)*deltar;
    fprintf(tmp_fp,"%.3lf %.3lf %i\n",thisr,Rcount[i]?theta[i]/Rcount[i]-0.5:0.0,Rcount[i]);
  }
  fclose(tmp_fp);
  fprintf(stdout,"Order parameter profile written to %s.\n",opf);
}
